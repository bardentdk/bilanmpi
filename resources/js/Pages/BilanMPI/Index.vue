...
const isAllSelected = computed(() => {
    return props.bilans.data.length > 0 && 
           selectedBilans.value.length === props.bilans.data.length;
});
...
const toggleSelectAll = () => {
    if (isAllSelected.value) {
        ...
        selectedBilans.value = props.bilans.data.map(bilan => bilan.id);
    }
};

const toggleSelectBilan = (bilanId) => {
    const index = selectedBilans.value.indexOf(bilanId);
    if (index > -1) {
        selectedBilans.value.splice(index, 1);
    } else {
        selectedBilans.value.push(bilanId);
    }
};

const isBilanSelected = (bilanId) => {
    return selectedBilans.value.includes(bilanId);
};

// Export de la sélection - VERSION CORRIGÉE AVEC GESTION D'ERREUR
const exportSelected = () => {
    if (selectedBilans.value.length === 0) {
        alert('Veuillez sélectionner au moins un bilan à exporter.');
        return;
    }
    
    if (selectedBilans.value.length > 50) {
        alert('Maximum 50 bilans par export.');
        return;
    }
    
    const message = `Vous allez télécharger ${selectedBilans.value.length} bilan(s) sélectionné(s).\n\nContinuer ?`;
    ...
    try {
        ...
        form.method = 'POST';
        form.action = route('bilans-mpi.export.selected');
        ...
        // Ajouter les IDs des bilans sélectionnés
        selectedBilans.value.forEach(bilanId => {
            const input = document.createElement('input');
            input.type = 'hidden';
            input.name = 'bilan_ids[]';
            input.value = bilanId;
    ...
    <div class="min-h-screen bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50 py-8 px-4 sm:px-6 lg:px-8">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="bg-white rounded-2xl shadow-xl p-8 mb-6">
                ...
                <div v-if="bilans.data.length > 0" class="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4 p-4 bg-gray-50 rounded-lg border border-gray-200">
                    <div class="flex items-center gap-4">
                        <!-- Bouton Tout sélectionner -->
                        <button
                            @click="toggleSelectAll"
                            class="flex items-center px-4 py-2 bg-white border-2 border-gray-300 rounded-lg hover:bg-gray-50 transition font-medium text-sm"
                            :class="isAllSelected ? 'border-blue-500 bg-blue-50 text-blue-700' : 'text-gray-700'"
                        >
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="text-sm font-medium text-gray-700">
                            <span v-if="selectedBilans.length > 0" class="text-blue-600 font-bold">
                                {{ selectedBilans.length }} sélectionné(s)
                            </span>
                    ...
                    <button
                        ...
                        :disabled="selectedBilans.length === 0 || isExporting"
                        class="px-6 py-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white font-semibold rounded-lg shadow-lg hover:from-green-700 hover:to-emerald-700 transition-all duration-200 flex items-center disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                        ...
                        <span v-if="!isExporting">Exporter (ZIP)</span>
                        <span v-else>Export en cours...</span>
                    </button>
                </div>
            </div>

            <!-- Liste des bilans avec checkboxes -->
            <div v-if="bilans.data.length > 0" class="space-y-4">
                <div 
                    ...
                    :class="isBilanSelected(bilan.id) ? 'ring-2 ring-blue-500' : ''"
                >
                    <div class="p-6">
                        <div class="flex items-start gap-4">
                            ...
                            <div class="flex items-center pt-1">
                                <input
                                    type="checkbox"
                                    :checked="isBilanSelected(bilan.id)"
                                    @change="toggleSelectBilan(bilan.id)"
            ...
            <div v-if="bilans.last_page > 1" class="mt-6 bg-white rounded-xl shadow-lg p-4">
                <div class="flex items-center justify-between">
                    ...
                    <span class="text-sm text-gray-600">
                        Page {{ bilans.current_page }} sur {{ bilans.last_page }} • 50 bilans/page
                    </span>
                    <Link
